...
При использовании в качестве входных данных .csv (после обработки файла lent3 в программе `Korr_v49`), необходимо убедиться что разделитель разрядов **точка**, а не **запятая**, в противном случае программа зависнет
## Интерфейс программы
...
Символ `$` служит экраном и он **обязателен**.
...
| mode-name: Обесточивание ATWS | название режима (исходного  события) |
...
```source
  - ДАВЛЕНИЕ НА ВХОДЕ В АКТИВНУЮ ЗОНУ
...
```source
  - $(ДАВЛЕНИЕ НА ВХОДЕ В АКТИВНУЮ ЗОНУ)/(1000000.0)$
  - $(ДАВЛЕНИЕ НА ВЫХОДЕ ИЗ АКТИВНОЙ ЗОНЫ)/(1000000.0)$
  - ''
  - ''
  - ''
  - ''
